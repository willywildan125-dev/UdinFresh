...
export default function HeroBanner() {
  ...
  return (
    <div className={`relative rounded-2xl overflow-hidden shadow-sm h-48 md:h-[280px] mb-8 mx-4 md:mx-6 md:mt-6 transition-colors duration-1000 ${slides[currentSlide].bgClass}`}>
      ...
      {slides.map((slide, index) => (
        ...
        >
          {/* Gambar Background dengan Filter hue-rotate untuk variasi warna */}
          <img 
            src={slide.image} 
            alt={slide.title} 
            className={`absolute inset-0 w-full h-full object-cover opacity-80 mix-blend-overlay ${slide.filter}`}
          />
          ...
          <div className="relative h-full flex flex-col justify-center p-6 md:p-12">
            ...
            <p className="text-gray-200 text-sm md:text-lg mb-6 max-w-[200px] md:max-w-xs">
              {slide.subtitle}
            </p>
