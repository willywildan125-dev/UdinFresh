...
export default function AdminDashboardPage() {
  const stats = [
    { title: 'Total Produk', value: '1,248', change: '+12%', icon: 'M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10M4 7v10l8 4' },
    { title: 'Total Pesanan', value: '8,592', change: '+5.4%', icon: 'M16 11V7a4 4 0 00-8 0v4M5 9h14l1 12H4L5 9z' },
    { title: 'Pesanan Diproses', value: '142', change: 'Stabil', isNeutral: true, icon: 'M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-6 9l2 2 4-4' },
    { title: 'Total Pendapatan', value: 'Rp 142.5M', change: '+24%', icon: 'M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z' }
  ];

  const recentTransactions = [
    { id: '#ORD-9982', customer: 'Budi Santoso', status: 'Completed', amount: 'Rp 450.000', color: 'bg-emerald-100 text-emerald-700' },
    { id: '#ORD-9981', customer: 'Siti Aminah', status: 'Processing', amount: 'Rp 1.250.000', color: 'bg-amber-100 text-amber-700' },
    { id: '#ORD-9980', customer: 'Ahmad Dahlan', status: 'Completed', amount: 'Rp 85.000', color: 'bg-emerald-100 text-emerald-700' },
    { id: '#ORD-9979', customer: 'Dewi Lestari', status: 'Pending', amount: 'Rp 210.000', color: 'bg-gray-100 text-gray-700' }
  ];
    ...
    <div className="space-y-6 max-w-7xl mx-auto">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="text-xs text-gray-400">
          Last updated: Today, 09:41 AM <span className="inline-block ml-1 cursor-pointer hover:text-emerald-500">↻</span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100 lg:col-span-2">
          <div className="flex items-center justify-between mb-6">
            <div>
              ...
              <p className="text-xs text-gray-400 mt-1">Revenue over the last 14 days</p>
            </div>
        ...
        <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100 flex flex-col">
          ...
          <div className="flex-1 space-y-5">
            {[
              { name: 'Sayuran Segar', val: 46, color: 'bg-emerald-500' },
              { name: 'Buah-buahan', val: 30, color: 'bg-teal-400' },
              { name: 'Biji-bijian & Beras', val: 14, color: 'bg-sky-400' },
              { name: 'Lainnya', val: 10, color: 'bg-gray-300' }
            ].map((cat) => (
              <div key={cat.name}>
                ...
                <div className="w-full bg-gray-100 rounded-full h-1.5">
                  <div className={`${cat.color} h-1.5 rounded-full`} style={{ width: `${cat.val}%` }}></div>
                </div>
              </div>
            ))}
          </div>
          <button className="w-full mt-6 py-2 border border-emerald-500 text-emerald-600 text-xs font-bold rounded-xl hover:bg-emerald-50 transition-colors">
            View Full Report
          </button>
        </div>
      </div>
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
        <div className="px-6 py-5 border-b border-gray-100 flex justify-between items-center">
          ...
          <a href="#" className="text-xs font-bold text-emerald-600 hover:text-emerald-700">View All</a>
        </div>
              ...
              {recentTransactions.map((tx) => (
                <tr key={tx.id} className="hover:bg-gray-50 transition-colors">
                  ...
                  <td className="px-6 py-4">
                    <span className={`inline-block px-2.5 py-1 rounded-full text-[10px] font-bold ${tx.color}`}>
                      {tx.status}
                    </span>
                  </td>
